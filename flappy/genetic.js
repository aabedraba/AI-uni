...
GeneticAlgorithm.prototype = {
	...
	crossOver : function(parentA, parentB) {
		var division = parentA.neurons.length / 2;
		var primerPunto = this.random(1, division-1); //punto izquierdo no es igual a 0
		var segundoPunto = this.random(division+1, parentA.neurons.length-2); //punto derecho no es igual al tamaño
		for (var i = primerPunto; i <= segundoPunto; i++ ) {
			var auxiliar = parentA.neurons[i]['bias'];
			parentA.neurons[i]['bias'] = parentB.neurons[i]['bias'];
			parentB.neurons[i]['bias'] = auxiliar;
		}
